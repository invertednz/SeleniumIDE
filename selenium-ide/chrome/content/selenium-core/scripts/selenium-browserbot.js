...
BrowserBot.prototype._registerAllLocatorFunctions = function() {
    ...
    this.locationStrategies['implicit'] = function(locator, inDocument, inWindow) {
        ...
        var labelTextNode = this.findElementBy('labeltext', locator, inDocument, inWindow);//this.locateElementByLabelText();
...
BrowserBot.prototype.locateElementByLabelText = function(labelText, inDocument, inWindow) {
    //var pageTitle = inDocument.getElementsByTagName('title').item(0).nodeName;
    
    LOG.warn("testing return Node");
    ...
    //LOG.warn("page source we are using = " + pageSource);
    //pageSource = inDocument.documentElement.outerHTML;
    //var pageSource = removeHiddenNodesAndGetSource(inDocument);
    
    LOG.warn("I have the page source");
    var xpathValue = "";
    
    try{	
    	  xpathValue = window['inst'].findElementUsingName(labelText, pageTitle, pageSource);
    ...
    if (matches) {
        ...
        if(!isNaN(lastMatch)){
        	elementNumber = parseInt(lastMatch)-1;
        }
    }
    	

    LOG.warn("ELEMENT NUMBER = " + elementNumber);
    ...
    try{
	    if(xpathValue.startsWith("exactxpath.")){
		    var defaultMsg  = "xpath.";
		    defaultMsg      = defaultMsg.replace (/([\!\$\(\)\*\+\.\/\:\=\?\[\\\]\^\{\|\}])/g, "\\$1")
		    var zRegEx      = new RegExp (defaultMsg, '');
		    xpathValue = xpathValue.replace(zRegEx, "");
		    
		return this.locateElementByXPath(xpathValue, inDocument);
	    }
	    if(xpathValue.startsWith("xpath.")){
	    		    var defaultMsg  = "xpath.";
	    		    defaultMsg      = defaultMsg.replace (/([\!\$\(\)\*\+\.\/\:\=\?\[\\\]\^\{\|\}])/g, "\\$1")
	    		    var zRegEx      = new RegExp (defaultMsg, '');
	    		    xpathValue = xpathValue.replace(zRegEx, "");
	    		    if(elementNumber>0){
	    		    	return this.locateElementsByXPath(xpathValue, inDocument)[elementNumber];
	    		    }
	    		return this.locateElementByXPath(xpathValue, inDocument);
	    }
	    if(xpathValue.startsWith("exactcss.")){
		var defaultMsg  = "css.";
		defaultMsg      = defaultMsg.replace (/([\!\$\(\)\*\+\.\/\:\=\?\[\\\]\^\{\|\}])/g, "\\$1")
		var zRegEx      = new RegExp (defaultMsg, '');
		xpathValue = xpathValue.replace(zRegEx, "");
		return this.locateElementByCss(xpathValue, inDocument);
	    }
	    if(xpathValue.startsWith("css.")){
	    		var defaultMsg  = "css.";
	    		defaultMsg      = defaultMsg.replace (/([\!\$\(\)\*\+\.\/\:\=\?\[\\\]\^\{\|\}])/g, "\\$1")
	    		var zRegEx      = new RegExp (defaultMsg, '');
	    		xpathValue = xpathValue.replace(zRegEx, "");
	    		if(elementNumber>0){
	    			return this.locateElementByCssForLabelText(xpathValue, inDocument, elementNumber);
	    		}
	    		return this.locateElementByCss(xpathValue, inDocument);
	    }
	    }catch(e){
...
/**
 * Find an element by css selector forLabelText
 */
BrowserBot.prototype.locateElementByCssForLabelText = function(locator, document, num) {
    var elements = eval_css(locator, document);
    if (elements.length != 0)
        return elements[num];
    return null;
