  ...
  for (var i = 0; i < LocatorBuilders.order.length; i++) {
    ...
    if (locator) {
      ...
      try {
        ...
        if(finderName=="labeltext"){
	        	LOG.debug("adding label text");
...
LocatorBuilders.add('labeltext', function(e) {
    try{
	  LOG.warn("Current Project = " +window['currentProject'] );
	LOG.warn("workingproject = " +window['workingproject'] );
	LOG.warn("working = " +window['working'] );
	oldOrder = LocatorBuilders.order;
	  LocatorBuilders.order = ['xpath:link', 'xpath:img', 'xpath:attributes',  'xpath:position'];
	  LOG.warn("Project error = "+ window['projectError']);
	  var locators = [];
	  locators = this.buildAll(e);  
	  LOG.warn("gets here");
	  LOG.warn("locator size = "+ locators.length);
	  //locators = getLocators(e);
	  var xpaths = [];
	  for (var i = 0; i < locators.length; i++) {
		LOG.warn("i = "+i);
		var locArray = locators[i];
		var locType = locArray[1];
		var locator = locArray[0];
		LOG.warn(locType);
		LOG.warn(String(locator));
		if(locType.startsWith("xpath:position")){
			if(xpaths.length<1){
				LOG.warn("adding xpath:position locator");
				xpaths[i] = locator;
			}
		}else{
			LOG.warn("Adding other locator");
			xpaths[i] = locator;
		}
	  }
	  //not sure if ownerDocument is correct
	  LOG.warn("finding name");
	  var doc = e.ownerDocument;
	  //var source = doc.documentElement.outerHTML
	  var source = doc.getElementsByTagName('html')[0].innerHTML
	  var origSource = doc.getElementsByTagName('html')[0].innerHTML
	  var title = doc.title;
	  var win = doc.defaultView
	  LOG.warn("about to load jQuery");
	  try{		
	  	      	var hidden = {};
	  		hidden.query = loadjQuery(win);
	  		doc = win.document;
	  		docClone = (hidden.query(doc).clone())[0];
	  		
	  		LOG.warn('number of doc elements = ' + docClone.getElementsByTagName('*').length);
	  		LOG.warn("about to load run each function");
	  		var elems = hidden.query(docClone.getElementsByTagName('*'));
	  		LOG.warn("got elems");
			LOG.warn("elems length = "+elems.length);
			elems = hidden.query(docClone.getElementsByTagName('*'));
			realElems = hidden.query(doc.getElementsByTagName('*'));
	  		LOG.warn("got elems");
			LOG.warn("elems length = "+elems.length);
	  		for (var j = 0; j < elems.length; j++) {
	  			var elem = elems[j];
	  			var realElem = realElems[j];
	  			var stylevalue = document.defaultView.getComputedStyle(realElem, "").getPropertyValue("display");
				var visiblevalue = document.defaultView.getComputedStyle(realElem, "").getPropertyValue("visibility");
			
 				if(stylevalue=="none"){
 					//LOG.warn("gets here and class = " + elem.className);
 					 elem.parentNode.removeChild(elem);
 					//hidden.query(elem).remove();
 					//hidden.query(doc.getElementsByTagName('*')).remove(elem);
 				}
				
				
	  		}
	  		
	      		//LOG.warn("finished removing elements");
	      		LOG.warn('number of doc elements now = ' + docClone.getElementsByTagName('*').length);
	      		
	      		
	      		source = docClone.getElementsByTagName('html')[0].innerHTML
	      		//LOG.warn(source);
	  }catch(e2){logExc(e2);}
	  
	  
	  //e.ownerDocument.getElementsByTagName('html')[0].innerHTML
	  var foundName = window['inst'].findElementsName(xpaths,title, source, origSource);
	  LOG.warn("name found ="+ foundName);
	  LocatorBuilders.order = oldOrder;//['labeltext', 'id', 'link', 'name', 'dom:name', 'xpath:link', 'xpath:img', 'xpath:attributes', 'xpath:href', 'dom:index', 'xpath:position'];
	  //return "labeltext="+foundName;
	  return foundName;
    }catch(exp){
    	LOG.warn("caught error trying to find name");
    	logExc(exp);
    	logExc(e);
    }
    LocatorBuilders.order = oldOrder;//['labeltext', 'id', 'link', 'name', 'dom:name', 'xpath:link', 'xpath:img', 'xpath:attributes', 'xpath:href', 'dom:index', 'xpath:position'];
...
function loadjQuery(wnd){
      try{
	      var loader = Components.classes["@mozilla.org/moz/jssubscript-loader;1"]
	       ...
	       var fullPath = 'file:///' + basePath + '/jquery-latest.js';

		 LOG.warn("jquery path  = "+fullPath);
		 loader.loadSubScript(fullPath,wnd);

	      //loader.loadSubScript("chrome://clhelper/content/jquery/jquery-1.5.js",wnd);
	      
	      var hidden = {};
		hidden.query = wnd.jQuery.noConflict(true);
